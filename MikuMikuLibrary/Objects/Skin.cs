...
public class Skin
{
    ...
    internal void Write(EndianBinaryWriter writer, BinaryFormat format)
    {
        ...
        writer.WriteOffsetIf(Bones.Any(x => x.Parent != null), 16, AlignmentMode.Center, () =>
        {
            foreach (var bone in Bones)
            ...
            foreach (var block in Blocks)
            {
                switch (block)
                {
                    case ConstraintBlock constraintBlock:
                    {
                        ...
                        {
                            var parentBoneInfo = GetBoneInfoByName(constraintBlock.ParentName) ??
                                                 FindParentBoneInfo(constraintBlock.ParentName, skeleton);
                    ...
                    case ExpressionBlock expressionBlock:
                    {
                        ...
                        {
                            var parentBoneInfo = GetBoneInfoByName(expressionBlock.ParentName) ??
                                                 FindParentBoneInfo(expressionBlock.ParentName, skeleton);
